...
describe("calculateCanvasSize", () => {
  ...
  test("should throw an error if length is non-numeric", () => {
    expect(() => calculateCanvasSize("rst", "100")).toThrow("Length should not be non-numeric");
  });

  test("should throw an error if width is non-numeric", () => {
    expect(() => calculateCanvasSize("10", "rst")).toThrow("Width should not be non-numeric");
  });
});